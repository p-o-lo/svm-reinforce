...
TIME_STEPS_TRAJ = 100   # time steps per batch (length of each trajectory)
CLIP = 0.2              # clip for surrogate loss of ppo
LAMBDA = 0.0            # for n-step bootstrap
GAMMA = 0.99            # discount factor
LR_ACTOR = 1e-4         # learning rate of the actor
LR_CRITIC = 1e-4        # learning rate of the critic
NUM_UPDATE = 5          # num update per step
...
class PPO_agent():
    ...
    def __init__(self, env, state_size, action_size, seed,
                ...
                time_steps_per_traj=TIME_STEPS_TRAJ,
                clip=CLIP,
                lambda_gae=LAMBDA,
                gamma=GAMMA,
                lr_critic=LR_ACTOR,
    ...
    def collect_trajs(self):
        """This is where we collect the trajectories of data from simulation. 
            Since this is an on-policy algorithm, we'll need to collect
            a fresh batch of data each time we iterate the actor/critic networks.
            ...
            Return
            ======
                traj_states: the observations collected this batch. Shape: (# of timesteps, dimension of observation)
                traj_acts: the actions collected this batch. Shape: (# of timesteps, dimension of action)
                traj_log_pol: the log of policiess of each action taken this batch. Shape: (# of timesteps)
                rew_t_future: the Rewards-To-Go of each timestep in this batch. Shape: (# of timesteps)
                len_traj: the lengths of each episode this batch. Shape: (# of traj (batch size))
        """
        ...
        # Run to collect trajs for a maximum of time_steps_per_traj
        for i in range(self.batch_size):
            ep_rews = []
            state = self.env.reset()
            done = False

            for t_traj in range(self.time_steps_per_traj):
                self.env.render()
                trajs_states.append(state)

                # Calculate action and log policy and perform a step of th env
                action, log_policy = self.act(state)
        ...
        rews_t_future = self.compute_return_fut(all_rews).to(device)
        trajs_log_pol = torch.tensor(trajs_log_pol, dtype=torch.float).to(device)

        return trajs_states, trajs_acts, trajs_log_pol, rews_t_future, len_trajs
    ...
    def step(self):
        # Collection of trajectories for training the networks
        trajs_states, trajs_acts, trajs_log_pol, rews_t_future, lens_trajs = self.collect_trajs()
